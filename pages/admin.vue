                ...
                <div v-else>
                    ...
                    <el-table :data="tableData" style="width: 100%">
                            ...
                            <template #default="scope">
                                <el-button v-if="tableData[scope.$index]!=null" link type="primary" size="small">
                                    <el-link target="_blank" :href="tableData[scope.$index].json">
                                        {{tableData[scope.$index].json.split("/")[tableData[scope.$index].json.split("/").length-1]}}
                                    </el-link>
                                </el-button>
                            </template>
                        </el-table-column>
                            ...
                            <template #default="scope">
                                <el-button v-if="tableData[scope.$index]!=null" link type="primary" size="small">
                                    <el-link target="_blank" :href="tableData[scope.$index].excel">
                                        {{tableData[scope.$index].excel.split("/")[tableData[scope.$index].excel.split("/").length-1]}}
                                    </el-link>
                                </el-button>
                            </template>
                        </el-table-column>
                        ...
                        <el-table-column fixed="right" label="开放/移除">
                            <template #default="scope">
                                <el-switch :model-value="switchValue[scope.$index]" class="ml-2" inline-prompt
...
watch(adminInfos, async (_value) => {
    await axios.get(useRuntimeConfig().public.apiUrl + "/UserData", { params: { id: adminInfos.id, passwd: adminInfos.passwd } }).then(({ data }) => {
        if (data.code) {
            datas.value = data.datas
            if (datas.value.length > 0) {
                datas.value.forEach((value: { json: string, excel: string, label: string, studentGrades: IstudentGrade[] }, _index: number) => {
                    let temp = {
                        label: value.label,
...
const isAdmin = async (id: string, passwd: string) => {
    await axios.get(useRuntimeConfig().public.apiUrl + "/IsAdmin", {
        ...
        if (data.code) {
            ...
            ElMessage({
                type: "info",
                message: "请注意：每次刷新界面都需要重新登录！"
            })
            adminInfos.id = formLabelAlign.id;
            adminInfos.name = data.data.name;
            adminInfos.passwd = formLabelAlign.passwd;
